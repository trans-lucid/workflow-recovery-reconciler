...
def build_status_report(store: Any) -> dict[str, Any]:
    ...
    for workflow in workflows:
        ...
        external_evidence = [attempt.get("evidence", {}) for attempt in attempts if attempt.get("action") == "external_status"]
        blocker = None
        if workflow["state"] == "needs_attention":
            blocker = workflow.get("last_error") or "external truth requires operator review"
        items.append(
            {
                ...
                "blocker": blocker,
                "next_action": workflow.get("recommended_action") or "inspect",
